...
def solve_prob_04_batch():
    # --- 図形データ定義 ---
    # 底辺 B(1,0), C(4,0) 幅3
    B, C = (1, 0), (4, 0)
    # 頂角A = 50度 の二等辺三角形 (外角130度から逆算)
    # 高さ = (底辺/2) / tan(頂角/2)
    # 50/2 = 25度
    height = (3 / 2) / np.tan(np.radians(25))
    A = (2.5, height)
    
    # DはABの延長線上にある（外角を作るため）
    # ベクトルBA = A - B
    ...
    # アニメーション1: 130°が赤くなる
    # ==========================================
    plot1 = SVGGeometryPlotter("prob04_1_130_red.svg")
    draw_base(plot1)
    
    # 静的要素
    plot1.add_angle_arc(B, C, A, "$x$", radius=0.5, fontsize=X_FONT, animate=False)
    
    # 130° (黒, 静的)
    plot1.add_angle_arc(A, C, D, "130°", radius=0.6, color='black', fontsize=BASE_FONT, text_color='black', animate=False)
    
    # 130° (赤, アニメーション)
    plot1.add_angle_arc(A, C, D, "130°", radius=0.6, color='red', fontsize=BASE_FONT, text_color='red', animate=True, delay=0.5)
    ...
    # ==========================================
    # アニメーション2: 青x追加済み → 各所に丸
    # ==========================================
    plot2 = SVGGeometryPlotter("prob04_2_circles.svg")
    draw_base(plot2)
    ...
    plot2.add_angle_arc(C, A, B, "$x$", radius=0.5, color='blue', fontsize=X_FONT, text_color='blue', animate=False)
    
    # アニメーション (丸つけ)
    plot2._track_points([A, B, C, D])
    r_x = plot2.get_circle_radius_for_text(X_FONT)
    r_num = plot2.get_circle_radius_for_text(BASE_FONT)
    
    pos_x_B = plot2.get_arc_text_pos(B, C, A, radius=0.5)
    pos_x_C = plot2.get_arc_text_pos(C, A, B, radius=0.5)
    pos_130 = plot2.get_arc_text_pos(A, C, D, radius=0.6)
    
    plot2.draw_circle(pos_x_B, radius=r_x, color='blue', lw=3, animate=True, delay=0.5)
    plot2.draw_circle(pos_x_C, radius=r_x, color='blue', lw=3, animate=True, delay=1.0)
    plot2.draw_circle(pos_130, radius=r_num, color='red', lw=3, animate=True, delay=1.5)
    ...
    # ==========================================
    # アニメーション3: 180° (半円) のアニメーション
    # ==========================================
    plot3 = SVGGeometryPlotter("prob04_3_180_arc.svg")
    draw_base(plot3)

    # Start状態
    plot3.add_angle_arc(B, C, A, "$x$", radius=0.5, fontsize=X_FONT, animate=False)
    plot3.add_angle_arc(C, A, B, "$x$", radius=0.5, fontsize=X_FONT, animate=False)
    # 角Aの外角 (130°) は青色で開始
    plot3.add_angle_arc(A, C, D, "130°", radius=0.6, color='blue', fontsize=BASE_FONT, text_color='blue', animate=False)
    ...
    # ==========================================
    # アニメーション4: 角ABCに70°赤記入 → 両底角に赤丸
    # ==========================================
    plot4 = SVGGeometryPlotter("prob04_4_answer_70.svg")
    draw_base(plot4)

    # Start状態: Cはすでに70° (黒)
    plot4.add_angle_arc(C, A, B, "70°", radius=0.5, fontsize=X_FONT, color='black', animate=False)
    
    # アニメーション1: 角ABCに赤で70°を表示
    plot4.add_angle_arc(
        B, C, A, 
        "70°", 
        radius=0.5, 
    ...
    # アニメーション2: 両方の70°に赤丸をつける
    pos_text_B = plot4.get_arc_text_pos(B, C, A, radius=0.5)
    pos_text_C = plot4.get_arc_text_pos(C, A, B, radius=0.5)
    r_circle = plot4.get_circle_radius_for_text(X_FONT)

    plot4.draw_circle(pos_text_B, radius=r_circle, color='red', lw=3, animate=True, delay=1.5)
    plot4.draw_circle(pos_text_C, radius=r_circle, color='red', lw=3, animate=True, delay=1.5)
...
if __name__ == "__main__":
    solve_prob_04_batch()
    print("All files generated in 'output_problem04'.")